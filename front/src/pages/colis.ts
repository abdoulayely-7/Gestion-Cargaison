...
async function displayColisInfo(colis: any, cargaison: any, infosAvancement: any): Promise<void> {
...
function getStatusColor(etat: string): string {
...
function getStatusTextColor(etat: string): string {
    switch (etat.toLowerCase()) {
...
function getStatusIcon(etat: string): string {
    switch (etat.toLowerCase()) {
...
function getProgressBarColor(etat: string): string {
    switch (etat.toLowerCase()) {
...
async function annulerColisAction(): Promise<void> {
    if (!currentColis) return;
...
async function afficherCarte(colis: any, cargaison: any): Promise<void> {
    try {
        if (!cargaison || !cargaison.lieuDepart || !cargaison.lieuArrivee) {
            return;
        }
        ...
        if (cargaison.lieuDepart.includes(',')) {
            const [lat, lng] = cargaison.lieuDepart.split(',').map((coord: string) => parseFloat(coord.trim()));
        ...
        if (cargaison.lieuArrivee.includes(',')) {
            const [lat, lng] = cargaison.lieuArrivee.split(',').map((coord: string) => parseFloat(coord.trim()));
            if (!isNaN(lat) && !isNaN(lng)) {
...
function calculateProgress(etat: string): number {
    switch (etat.toLowerCase()) {
...
function interpolatePosition(start: {lat: number, lng: number}, end: {lat: number, lng: number}, progress: number): {lat: number, lng: number} {
    return {

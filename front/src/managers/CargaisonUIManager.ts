...
export class CargaisonUIManager {
    ...
    private cargaisonsDisponibles: any[] = [];
    ...
    private async chargerCargaisonsDisponibles(): Promise<void> {
        this.cargaisonsDisponibles = await CargaisonService.obtenirCargaisonsDisponibles();
        this.remplirSelectCargaisons();
    }

    private remplirSelectCargaisons(): void {
        const select = document.getElementById('cargaison_existante') as HTMLSelectElement;
        if (select) {
    ...
    private configurerEventListeners(): void {
    ...
    private changerOptionCargaison(): void {
        const radioButtons = document.getElementsByName('option_cargaison') as NodeListOf<HTMLInputElement>;
        const selectionExistante = document.getElementById('selection_cargaison_existante');
        const typeCargaisonField = document.getElementById('type_cargaison') as HTMLSelectElement;
    ...
    private chargerInfosCargaison(): void {
        const select = document.getElementById('cargaison_existante') as HTMLSelectElement;
    ...
    private preremplirTypeCargaison(typeCargaison: string): void {
        const typeCargaisonSelect = document.getElementById('type_cargaison') as HTMLSelectElement;
    ...
    private cacherInfosCargaison(): void {
        const infoCargaison = document.getElementById('info_cargaison_selectionnee');
        infoCargaison?.classList.add('hidden');
    }

    getOptionSelectionnee(): string {
        const radioButtons = document.getElementsByName('option_cargaison') as NodeListOf<HTMLInputElement>;
        let optionSelectionnee = '';
        radioButtons.forEach(radio => {
            if (radio.checked) {
    ...
    getCargaisonSelectionnee(): number | null {
        const select = document.getElementById('cargaison_existante') as HTMLSelectElement;
        return select.value ? parseInt(select.value) : null;
    }

    getCargaisonsDisponibles(): any[] {

...
export class ColisMapService {
    ...
    async displayRoute(departureCoords: {lat: number, lng: number}, arrivalCoords: {lat: number, lng: number}, currentCoords: {lat: number, lng: number}, transportType: string): Promise<void> {
        if (!this.map) return;
